...
public interface SocketManager
{
    ...
    /**
     * Provide a means for intercepting messages. This allows clients of this
     * class to override normal behavior upon the receipt of data over the
     * socket.
     * 
     * @param interceptor
    ...
    /**
     * Set the default timeout when waiting for a message response from the
     * server. A value of <code>0</code> indicates no timeout.
     * 
     * @param time
    ...
    /**
     * Retrieve the current default timeout for waiting for a message response
     * from the server. A value of <code>0</code> indicates no timeout.
     * 
     * @param unit
    ...
    /**
     * Connect to a server. This method will block until the connection
     * completes. If a connection is already active, this method will do
     * nothing.
     * 
    ...
    /**
     * Determine whether or not this manager has an active connection to a
     * server.
     * 
     * @return <code>true</code> if the manager is connected; <code>false</code>
     *         otherwise
     */
     ...
     * @see #isConnected()
     * 
     * @return the connected address or <code>null</code> if this manager is not
     *         connected
     */
    public InetSocketAddress getConnectedAddress();

    /**
     * Create a copy of this manager that is connected to the same server (if
    ...
    /**
     * Close the active connection and clean up any resources associated with
     * it. If there is no active connection, this method will make sure that
     * resources are cleaned up and return.
     */
    public void disconnect();

    /**
     * Send a message over the active connection. This method will not wait for
     * a response and should not be used with messages that will trigger a
     * response from the server.
     * 
     * @param message
     *            the message to send
     * @throws IOException
     *             if this manager is not connected to a server, is interrupted
     *             while sending the message, or some other communication error
     *             occurs
     */
     ...
     * wait according to the {@link #getDefaultMessageTimeout(TimeUnit) default
    ...
    /**
     * Redirect the data channel used by this socket manager. While the channel
     * is redirected, normal communication is suspended. To release the
     * redirected channel, simply close it.
     * 
